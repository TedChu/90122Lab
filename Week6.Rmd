...
The map appears OK, but a closer look shows that some suburbs are not supposed to be there (e.g. Eltham). Here is a list of all suburbs in City of Banyule (https://en.wikipedia.org/wiki/City_of_Banyule). The reason for this is likely due to the different resolution and accuracy of shapefiles (VICSub and VICLGA). Therefore, I will remove all the suburbs with a very small area (Here, the unit is square meter, and 10000 is small by a suburb standard.)  
...
Next, we draw all suburbs in City of Banyule. Moreover, for suburbs only partly within Banyule, we want to draw the whole suburbs. 
```{r, echo=TRUE, fig.width=7, fig.align="center", message = FALSE}
## Names of all suburbs in Banyule
BSub$VIC_LOCA_2

ASub = subset(VICSub, VICSub$VIC_LOCA_2 %in% BSub$VIC_LOCA_2)
## There are two suburbs in Victoria callled "Bellfield" but only one is in City of Banyule. I am mannually remove the other "Bellfield" which is not inside City of Banyule. 
ASub = ASub[-5,]

## Draw the map
ggplot() + geom_sf(data = ASub, aes(fill = VIC_LOCA_2)) + 
  geom_sf_label(data = ASub, aes(label = VIC_LOCA_2), size = 2.1) + 
  geom_sf(data = Bcen, size = 4) + theme(legend.position="none")
```
